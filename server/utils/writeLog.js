...
let logger = {}
let _error = log4js.getLogger('error')
let _res = log4js.getLogger('response')
// 封装错误日志
logger.error = (ctx, err) => {
    if(ctx && err) {
        _error.error(formatError(ctx, err))
    }
}
// 封装响应日志
logger.res = (ctx, resTime) => {
    if(ctx) {
        _res.info(formatRes(ctx, resTime))
    }
}
